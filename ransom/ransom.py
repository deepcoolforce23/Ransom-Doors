...
RANSOM_TIME = 78
RANSOM_GOLD = 500
GLITCH_IMAGES = ['glitch1.png', 'glitch2.jpg', 'glitch3.jpg', 'glitch4.jpg', 'glitch5.jpg', 'glitch6.png', 'glitch7.jpg', 'stop_sign.png', 'glitch6.png', 'stop_sign.png', 'stop_sign.png', 'stop_sign.png', 'stop_sign.png']
...
class JumpscareWindow(QWidget):
    ...
    def __init__(self, image_path):
        ...
        self.progress_fill.setStyleSheet('background: transparent; border: none;')
        ...
        self.glitch_labels = []
        glitch_texts = ['DOWNLOADING...', 'D0WNLOAD1NG...', 'DOWNL0ADING...', 'DOWNLOAD_', 'DOWNLOAD ERROR', 'DOWNLOADING', 'D0WNLOAD', 'DOWNLOAD...', 'DOWNLOADING..', 'D0WNLOAD1NG', 'DOWNLOAD PAYLOAD', 'DOWNLOADING DATA']
        for _ in range(8):
    ...
    def _render_bar(self, fill_fraction):
        """Draw a chunky segmented red progress bar onto progress_fill."""
        bar_w = self.progress_fill.width()
        bar_h = self.progress_fill.height()
        ...
        filled_segs = int(fill_fraction * seg_count)
        partial = fill_fraction * seg_count - filled_segs

        red = QColor(220, 20, 20)
        for i in range(seg_count):
            x = i * (seg_w + gap)
            w = seg_w
            if i < filled_segs:
                painter.fillRect(int(x), 0, int(w), bar_h, red)
            elif i == filled_segs:
                pw = w * partial
                if pw > 0:
                    painter.fillRect(int(x), 0, int(pw), bar_h, red)

        painter.end()
        self.progress_fill.setPixmap(QPixmap.fromImage(img))
    ...
    def start_download(self):
        self.image.hide()
        # NOTE: static_timer keeps running during download.
        self.static_background.setStyleSheet('background-color: rgb(90, 0, 0);')
        self.static_background.show()
        self.static_background.lower()
        self.download_elapsed = 0
        self.download_progress = 0
        ...
        bx = (self.width() - bw) // 2
        by = (self.height() - bh) // 2
        self.progress_background.setGeometry(bx, by, bw, bh)
        self.progress_background.show()

        # progress_fill lives inside the 3px border of progress_background
        self.progress_fill.setGeometry(0, 0, bw - 6, bh - 6)
        self._render_bar(0.0)

        self.download_label.setText('DOWNLOADING...')
        self.download_label.adjustSize()
        self.download_label.move(
            (self.width() - self.download_label.width()) // 2,
            by - self.download_label.height() - 20,
        )
        self.download_label.show()

        # No percentage readout in this look
        self.percent_label.hide()
        ...
        # Make sure the main UI sits above the scattered glitch labels
        self.progress_background.raise_()
    ...
    def update_download(self):
        ...
        self.download_progress = min(100, self.download_elapsed / 2000 * 100)
        zoom = 1.0 + self.download_progress / 100 * 0.15
        base_width = 600
        base_height = 45
        ...
        if random.random() < 0.3:
            bar_x += random.randint(-4, 4)
        if random.random() < 0.3:
            bar_y += random.randint(-4, 4)
        ...
        if self.download_progress >= 100:
            ...
            visual_progress = self.download_progress
            if random.random() < 0.25:
                visual_progress = max(
                    0, min(99, visual_progress + random.randint(-10, 10))
                )
            self._render_bar(visual_progress / 100.0)

            if random.random() < 0.35:
                self.download_label.setText(random.choice([
                    'DOWNLOADING...', 'DOWNLOADING..', 'D0WNLOAD1NG...',
                    'DOWNL0ADING...', 'DOWNLOAD_', 'DOWNLOADING',
                    'D0WNLOAD', 'DOWNLOAD...', 'D0WNLOAD1NG', 'DOWNLOAD FILE',
                ]))
            font = self.download_label.font()
            font.setPointSize(max(1, int(22 * zoom)))
            font.setBold(True)
            self.download_label.setFont(font)
            self.download_label.adjustSize()

            jx = random.randint(-8, 8) if random.random() < 0.4 else 0
            jy = random.randint(-4, 4) if random.random() < 0.4 else 0
            self.download_label.move(
                (self.width() - self.download_label.width()) // 2 + jx,
                bar_y - self.download_label.height() - 20 + jy,
            )

            self.update_glitch_text()
            # Re-stack: glitch labels < progress bar < main label
            self.progress_background.raise_()
            self.download_label.raise_()

    def finish_download(self, bar_x, bar_y, bar_width, bar_height, zoom):
        self.download_timer.stop()
        self.progress_fill.setGeometry(0, 0, max(1, bar_width - 6), max(1, bar_height - 6))
        self._render_bar(1.0)
        self.download_label.hide()
        for label in self.glitch_labels:
            label.hide()
